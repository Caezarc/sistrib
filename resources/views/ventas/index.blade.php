                ...
                <div class="card-body">
                  ...
                  <table id="example1" class="table table-striped table-bordered nowrap">
                    ...
                    <tr>
                      ...
                      <td>{{$venta->ven_retgdi}}</td>
                      <td>{{$venta->ven_retalma}}</td>
                      <td>{{$venta->ven_iva}}</td>
                      <td>{{$venta->ven_total}}</td>
                      <td>
                        <a href="{{url('/ventas/'.$venta->id.'/edit') }}" class="btn btn-warning btn-sm"><i class="fas fa-pen"></i></a>
                      </td>  
                      <td>
                        <form action="{{url('/ventas/'.$venta->id) }}", method="post">
                          @csrf
                          {{method_field('DELETE')}}
                          <button type="submit" class="btn btn-outline-danger btn-sm" onclick="return confirm('¿Deseas Borrar este registro')"><i class="fas fa-trash-alt"></i></button>
                        </form>
                      </td>
                    </tr>
                    @endforeach
